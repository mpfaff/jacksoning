...
public interface DynamicMemoize {
	...
	<{{ char_range(from='A', n=total) | delimit(sep=", ", post=", ") }}R> R get({% for c in char_range(from='A', n=tracked) %}{{ c }} {{ c | lower }}, {% endfor %}Computer.By{{ total }}<{{ char_range(from='A', n=total) | delimit(sep=", ", post=", ") }}R> func{% for c in char_range(from=(char_add(c='A', i=tracked)), n=untracked) %}, {{ c }} {{ c | lower }}{% endfor %});
	{%- endfor %}
	{%- endfor %}
